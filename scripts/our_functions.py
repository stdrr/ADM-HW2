...
def complete_funnels(columns:list):
    ...
    n_carts = carts.shape[0]
    purchases_carts = purchases.merge(carts, on=['user_session', 'product_id'], how='left', indicator=True)
    del purchases, carts
    n_one_click = purchases_carts[purchases_carts._merge == 'left_only'].shape[0]
    del purchases_carts
...
def average_number_operations(columns:list):
    ...
    for iterator in dataset_iterators:
        for dataset in iterator:
            n_views += dataset[dataset.event_type == 'view'].shape[0]
            n_carts += dataset[dataset.event_type == 'cart'].shape[0]
            n_purchases += dataset[dataset.event_type == 'purchase'].shape[0]
            sessions_list.append(dataset['user_session'].drop_duplicates())
            del dataset
    n_sessions = pd.concat(sessions_list, ignore_index=True).drop_duplicates().shape[0]
...
def probability_purchase(columns:list):
    ...
    n_carts = carts.shape[0]
    del carts, purchases
    n_purchased = carts_purchases[carts_purchases._merge == 'both'].shape[0]
    print(f'The probability that products added once to the cart are effectively bought is {round(n_purchased / n_carts * 100, 2)}%.')



## ***** What’s the average time an item stays in the cart before being removed?
def average_time_cart(columns:list):
    global month_files, dtype
    time_deltas = 0
    total = 0
    for month in month_files:
        dataset = pd.read_csv(DATA_PATH + month, usecols=columns, dtype=dtype, parse_dates=['event_time'], date_parser=pd.to_datetime)
        carts = dataset[dataset.event_type == 'cart'][['user_session', 'product_id', 'event_time']]
        purchases = dataset[dataset.event_type == 'purchase'][['user_session', 'product_id', 'event_time']]
        last_session_op = dataset.groupby('user_session').event_time.max().reset_index()
        del dataset
        pending_carts = carts.merge(purchases, on=['user_session', 'product_id'], how='left', indicator=True)
        pending_carts = pending_carts[pending_carts._merge == 'left_only'][['user_session', 'event_time_x']]
        del carts, purchases
        pending_carts = pending_carts.merge(last_session_op, on='user_session')
        for time, max_time in zip(pending_carts['event_time_x'], pending_carts['event_time']):
            time_deltas += (max_time - time).total_seconds()
        total += pending_carts.shape[0]
        del pending_carts, last_session_op
    print(f'On average, an item stays in the cart {round(time_deltas / total):,.2f} seconds before being removed.')
...
def average_time_after_first_view_1(columns:list):
    ...
    mean = round(sum_deltas / joined.shape[0], 2)
    st_dev = 0
    for t_views, t_purchases_carts in zip(joined['event_time_views'], joined['event_time_purchases_carts']):
        st_dev += ((t_purchases_carts - t_views).total_seconds() - mean)**2
    st_dev = round(np.sqrt(st_dev / (joined.shape[0] - 1)), 2) 
...
def average_time_after_first_view_2(columns:list):
    ...
    for month in month_files:
        user_views_p_min_list = []
        user_purchases_carts_p_min_list = []
        iterator = load_data([month,], columns, chunk=True, chunksize=2*10**7, parse_dates=['event_time'])[0]
        for dataset in iterator:
        ...
        for t_views, t_purchases_carts in zip(joined['event_time_views'], joined['event_time_purchases_carts']):
            sum_deltas += (t_purchases_carts - t_views).total_seconds()
        total += joined.shape[0]
        del user_views_min, user_purchases_carts_min, joined
    mean = round(sum_deltas / total, 2)
    print(f'The average time between the first view time and a purchase/addition to cart is {round(mean / 60, 2)} minutes.') # we convert the time from seconds to minutes
...
def num_sold_per_cat():
    ...
    data.category_code = data.category_code.map(extract_cat)
    plt.figure(figsize=(15,15))
    plt.subplot(title='October')
    data[data.event_type=='purchase'].groupby(data.category_code).product_id.count().sort_values(ascending=False).head(10).plot.barh()
    del data
    plt.show()
    ...
    data2.category_code = data2.category_code.map(extract_cat)
    plt.subplot(title='November')
    data2[data2.event_type=='purchase'].groupby(data2.category_code).product_id.count().sort_values(ascending=False).head(10).plot.barh()
    del data2
    plt.show()
...
def most_v_cat():
    ...
    extract_cat = lambda x: '.'.join(x.split('.')[:2])
    data.dropna(inplace=True)
    data.category_code = data.category_code.map(extract_cat)
    plt.figure()
    plt.subplot(title='October')
    data[data.event_type=='view'].groupby('category_code').category_code.count().nlargest(10).plot.barh()
    plt.show()
    del data
    ...
    data2.category_code = data2.category_code.map(extract_cat)
    plt.subplot(title='November')
    data2[data2.event_type=='view'].groupby('category_code').category_code.count().nlargest(10).plot.barh()
    plt.show()
    del data2
...
def most_sold_per_cat():
    ...
    fr=[data,data2]
    data_tot=pd.concat(fr,ignore_index=True).dropna()
    
    # obtain a subset of only the purchases,
    p=data_tot[data_tot.event_type=='purchase']
    extract_cat = lambda x: x.split('.')[0]
    p.category_code = p.category_code.map(extract_cat)
    ...
    for category_code, t in  p.groupby('category_code'):
        print(category_code)
        print(t.groupby('product_id').event_type.count().nlargest(10))
...
def avg_price_by_brand():
    ...
    dstot = pd.concat([ds10, ds11], ignore_index=True).dropna().drop_duplicates()
    del ds10, ds11

    extract_cat = lambda x: x.split('.')[0]
    dstot.category_code = dstot.category_code.map(extract_cat)
    
    dstot_grouped2 = dstot.groupby(["category_code", "brand"])["price"].mean().nlargest(10)
...
def avg_price_by_brand_cat():
    ...
    cat = input()
    dstot_grouped1 = dstot[dstot["category_code"]==cat].groupby("brand")["price"].mean().nlargest(50)
    dstot_grouped1.plot.bar(figsize=(18, 10), title="Top 50 more expensive brands")
    plt.show()
...
def highest_avg_price_brand():
    ...
    dstot_grouped2 = dstot.groupby(["category_code", "brand"])["price"].mean().reset_index()
    print(dstot_grouped2.iloc[dstot_grouped2.groupby("category_code").price.idxmax()].sort_values(by='price'))
...
def havg_visit():
    ...
    #plot the results in 2 subplots heatmaps where it's easy to visualize in which part of the week the website is most visited 
...
def purchases_cat():
    ...
    dstot_grouped = dstot[dstot["event_type"]=="purchase"].groupby("category_code")["event_type"].count().nlargest(50)
    dstot_grouped.plot.bar(figsize=(18, 10), title="Top 50 categories")
    plt.show()
